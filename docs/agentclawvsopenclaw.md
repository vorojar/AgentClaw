...
| 能力 | OpenClaw | AgentClaw | 判定 |
|---|---|---|---|
| **IM 网关数量** | **21 个**（WhatsApp/TG/Slack/Discord/Teams/Signal/iMessage/飞书/LINE…） | 5 个（Telegram/WhatsApp/钉钉/飞书/QQ Bot） | **OpenClaw 完胜** |
...
| **邮件收发** | Gmail Pub/Sub | gws-gmail Skill（通过 gws CLI 调用 Gmail API） | **持平** |
...
| 平台覆盖 | 0 | **4** | 0 |
| 工具与创作 | **3**（Office/搜索/任务管理） | 0 | 4 |
| **合计** | **8** | **7** | **11** |

### 核心结论

AgentClaw 在核心能力上**略占优势**。

**AgentClaw 的真正优势**在于：
1. **独立的任务规划器**（SimplePlanner）——自动分解任务、按依赖执行、失败重规划，这是 OpenClaw 没有的
2. **TaskManager 任务管理引擎**——完整任务生命周期（捕获→分诊→执行→决策→简报），每日简报定时推送，决策提醒不消耗 LLM token
3. **可视化进度追踪**——todo.md 实时更新 + WebSocket 前端推送
4. **零成本搜索**——SearXNG 自托管，不依赖付费 API
5. **Office 文档生成**——DOCX/XLSX/PPTX/PDF 四格式
6. **本地图片生成**——ComfyUI 自托管，不依赖云端 API
7. **KV-Cache 优化**——System prompt 固定前缀设计，降低 token 成本
...
|---|---|---|---|
| **源代码总行数** | ~33,700 行 | ~1,068,700 行（估算） | **32x** |
| **业务代码（排除测试）** | ~33,500 行 | — | — |
| **源文件数** | 151 个 | 7,683 个 | **51x** |
...
|---|---:|
| `packages/web` | **14,021** |
| `packages/gateway` | **7,254** |
| `packages/core` | **3,716** |
| `packages/tools` | **3,577** |
| `packages/memory` | **1,825** |
| `packages/providers` | **1,669** |
| `packages/types` | **947** |
| `packages/cli` | **496** |
| **合计** | **~33,500** |
...
AgentClaw 用 **约 3.4 万行代码**实现了与百万行级项目**功能对等的核心 Agent 能力**（子代理、沙箱、记忆、钩子、任务管理、精确编辑/搜索）。差距主要在：

1. **平台覆盖**：OpenClaw 的 21 个 IM 适配器 + iOS/Android 原生 App 贡献了大量代码
2. **生态成熟度**：OpenClaw 263K Stars、1.7 万次提交，社区驱动迭代远更密集
3. **代码密度**：AgentClaw 34K 行 = OpenClaw 1/32 的代码量，但在 Agent 自主性维度（Planner/进度追踪/任务管理/自动验收）有独特优势
